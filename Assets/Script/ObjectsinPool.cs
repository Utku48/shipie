...
public class ObjectsinPool : MonoBehaviour
{
    ...
    IEnumerator SpawnObjectsWithDelay()
    {
        ...
        for (int i = 0; i < temp; i++)
        {
            ...
            GameObject obj = objectPool.GetPooledObject(counter++ % 5); // ObjectPool scriptinden GetPooledObject'i çağır


            int randomIndex = Random.Range(0, PrefabSpawnPos.Length);
            Vector3 spawnPosition = new Vector3(PrefabSpawnPos[randomIndex].position.x, 0.4f, PrefabSpawnPos[randomIndex].position.z);
